...
def forward(inputs, targets, memory):
    ...
    wes, hs, cs ,xs, os, ps, ys, zs, f_gate, i_gate, o_gate, cs_hat = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
    ...
    for t in range(len(inputs)):
        ...
        cs_hat[t] = np.tanh (np.dot(Wc, zs[t]) + bc)

        # new memory: applying forget gate on the previous memory
        # and then adding the input gate on the candidate memory
        # c_new = f_gate * prev_c + i_gate * \hat{c}
        cs[t] = f_gate[t] * cs[t-1] + i_gate[t] * cs_hat[t]
    ...
    activations = (xs, cs, hs, os, ps, ys, o_gate, i_gate, f_gate, cs_hat, zs)
...
def backward(activations, clipping=True):
    ...
    xs, cs, hs, os, ps, ys, o_gate, i_gate, f_gate, cs_hat, zs = activations
    ...
    Wf_h = Wf[:,0:hidden_size]
    Wf_x = Wf[:,hidden_size:concat_size]
    Wi_h = Wf[:,0:hidden_size]
    Wi_x = Wf[:,hidden_size:concat_size]
    Wo_h = Wf[:,0:hidden_size]
    Wo_x = Wf[:,hidden_size:concat_size]
    Wc_h = Wf[:,0:hidden_size]
    Wc_x = Wf[:,hidden_size:concat_size]
    ...
    for t in reversed(range(len(inputs))):
        ...
        do_gate = dhnext * np.tanh(cs[t])
        dvor_o_gate = dsigmoid(o_gate[t]) * do_gate
        
        dWo += np.dot(dvor_o_gate,zs[t].T)
        dbo += dvor_o_gate
        
        dtanh_c_next = dhnext * o_gate[t]
        dc_next = dtanh_c_next * dtanh(cs[t])
        dc_hat = dc_next * i_gate[t]
        dvor_c_hat = dtanh(cs_hat[t]) * dc_hat
        
        dWc += np.dot(dvor_c_hat,zs[t].T)
        dbc += dvor_c_hat
        
        di_gate = dc_next * cs_hat[t]
        dvor_i_gate = dsigmoid(i_gate[t]) * di_gate
        
        dWi += np.dot(dvor_i_gate,zs[t].T)
        dbi += dvor_i_gate
        
        df_gate = dc_next * cs[t-1]
        dvor_f_gate = dsigmoid(f_gate[t]) * df_gate
        
        dWf += np.dot(dvor_f_gate,zs[t].T)
        dbf += dvor_f_gate
        
        dhnext = (np.dot(dvor_o_gate.T,Wo_h) + np.dot(dvor_i_gate.T,Wi_h) + np.dot(dvor_f_gate.T,Wf_h) + np.dot(dvor_c_hat.T,Wc_h)).T
        dx = np.dot(Wo_x.T,dvor_o_gate) + np.dot(Wi_x.T,dvor_i_gate) + np.dot(Wf_x.T,dvor_f_gate) + np.dot(Wc_x.T,dvor_c_hat)
...
def sample(memory, seed_ix, n):
    ...
    for t in range(n):
        ...
        c = np.multiply(f_gate,c) + np.multiply(i_gate , c_hat)
